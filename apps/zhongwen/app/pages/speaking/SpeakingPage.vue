      ...
      if (word) {
        ...
        if (correct) {
          ...
          if (hasElevenLabs) {
            ...
            await speakWithElevenLabs(word.hanzi).finally(() => { isSpeaking.value = false; });
          }
        }
      }
    } catch (e) {
...
async function sendChatToPython(blob: Blob) {
  try {
    ...
    const response = await $fetch<{ user_text: string; hanzi: string; pinyin: string; translation: string }>("http://localhost:8000/chat", {
      method: "POST",
      body: formData,
    });
    ...
    if (response.user_text) {
      ...
      if (hasElevenLabs && response.hanzi) {
        isSpeaking.value = true;
        await speakWithElevenLabs(response.hanzi).finally(() => { isSpeaking.value = false; });
      }
    }
...
<template>
  <div class="flex min-h-screen flex-col items-center px-3 py-4 sm:px-4 sm:py-8">
    ...
    <div v-else-if="isDictList" class="w-full max-w-xl">
      ...
      <p class="mb-4 text-xs text-muted-foreground sm:mb-6 sm:text-sm">Select a dictionary to practise pronunciation.</p>
    ...
    <div v-else-if="isSentences" class="flex w-full max-w-md flex-col gap-4 h-[85vh]">
      <div class="flex items-center gap-2 sm:gap-3 shrink-0">
        <BackButton />
        <h1 class="text-xl font-semibold text-foreground sm:text-2xl">AI Interview</h1>
      </div>
      
      <div class="flex-1 rounded-2xl border border-border bg-card p-4 flex flex-col overflow-hidden relative shadow-sm">
        
        <div ref="chatContainer" class="flex-1 overflow-y-auto space-y-5 mb-4 pr-2 pb-4 scroll-smooth">
          <div v-if="chatHistory.length === 0" class="text-center text-muted-foreground mt-20 flex flex-col items-center gap-3">
          ...
          <div v-for="(msg, index) in chatHistory" :key="index" class="flex flex-col w-full">
            <div v-if="msg.role === 'user'" class="self-end flex flex-col items-end max-w-[85%]">
              <span class="text-[10px] text-muted-foreground mb-1 mr-1 uppercase tracking-wider font-semibold">You</span>
              <div class="px-4 py-2.5 bg-accent text-white rounded-2xl rounded-br-sm shadow-sm">
                {{ msg.parts }}
              </div>
            </div>
            
            <div v-else class="self-start flex flex-col items-start max-w-[90%]">
              <span class="text-[10px] text-muted-foreground mb-1 ml-1 uppercase tracking-wider font-semibold">Professor</span>
              <div class="px-4 py-3 bg-muted border border-border/50 rounded-2xl rounded-bl-sm shadow-sm flex flex-col gap-1.5">
                <span class="text-xl font-medium text-foreground tracking-wide leading-relaxed">{{ msg.parts }}</span>
                <span class="text-sm text-muted-foreground leading-snug">{{ msg.pinyin }}</span>
                <span class="text-xs text-muted-foreground leading-snug">{{ msg.translation }}</span>
              </div>
            </div>
          </div>
          
          <div v-if="isChatLoading" class="self-start flex flex-col items-start max-w-[80%] mt-4">
            <div class="px-4 py-3 bg-muted rounded-2xl rounded-bl-sm flex items-center gap-2">
              <Icon icon="mdi:dots-horizontal" class="h-5 w-5 animate-pulse text-muted-foreground" />
            </div>
          </div>
        </div>

        <p v-if="chatApiError" class="text-xs text-red-400 text-center mb-2 shrink-0">{{ chatApiError }}</p>

        <div class="pt-3 border-t border-border/60 shrink-0 flex flex-col items-center bg-card">
            <button
              type="button"
              class="flex h-16 w-16 items-center justify-center rounded-full border-2 transition-all duration-300 disabled:opacity-40 shadow-sm"
              :class="{
                'scale-105 border-red-500 bg-red-500/10 text-red-500 animate-pulse': isChatRecording,
                'border-accent bg-accent/10 text-accent hover:bg-accent/20 hover:scale-105': !isChatRecording,
            ...
            <p class="text-center text-[11px] text-muted-foreground mt-2.5 uppercase tracking-wider font-medium">
    ...
    <div v-else-if="isPractice" class="flex w-full max-w-md flex-col gap-4">
      ...
      <template v-else-if="currentWord">
        <div class="relative flex flex-col items-center gap-5 rounded-2xl border bg-card px-8 py-10 text-center shadow-sm transition-colors duration-300"
          ...
          <div class="flex flex-col items-center gap-2 min-h-[40px]">
             ...
             <button v-if="userAudioUrl && lastResult" @click="playUserAudio" class="flex items-center gap-1.5 text-xs font-medium text-accent hover:text-accent/80 transition-colors bg-accent/10 px-3 py-1.5 rounded-full">
          ...
          <div class="mt-2 flex items-center gap-4">
            <button type="button" class="flex h-12 w-12 items-center justify-center rounded-full border border-border bg-muted text-foreground transition-colors hover:bg-muted/70 disabled:opacity-40"
              :disabled="!hasElevenLabs || isSpeaking" aria-label="Play pronunciation" @click="speak">
              <Icon v-if="isSpeaking" icon="mdi:loading" class="h-5 w-5 animate-spin" />
              <Icon v-else icon="lucide:volume-2" class="h-5 w-5" />
            </button>
            <button v-if="!lastResult" type="button" class="flex h-16 w-16 items-center justify-center rounded-full border-2 transition-all duration-200 disabled:opacity-40"
              :class="{'scale-110 border-red-500 bg-red-500/20 text-red-400': isRecording, 'border-accent bg-accent/10 text-accent hover:bg-accent/20': !isRecording && !isTranscribing, 'border-muted bg-muted text-muted-foreground': isTranscribing}"
              :disabled="isTranscribing" aria-label="Start recording" @click="toggleRecording">
            ...
            <button v-if="lastResult" @click="nextWord" class="flex h-12 w-12 items-center justify-center rounded-full border-2 border-accent bg-accent text-white hover:bg-accent/90 transition-all duration-200 shadow-md hover:scale-105">
          ...
          <p class="-mt-1 text-xs text-muted-foreground">
            ...
            <template v-else-if="lastResult">Tap <Icon icon="lucide:arrow-right" class="inline h-3 w-3"/> to continue</template>
