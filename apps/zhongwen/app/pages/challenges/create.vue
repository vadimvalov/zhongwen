...
async function submit() {
  ...
  try {
    ...
    const result = await $fetch<{ id: string; inviteCode: string }>("/api/challenges", {
      method: "POST",
      body: {
    ...
    router.push(`/challenges/${result.inviteCode}`);
  } catch (err: any) {
  ...
  <div class="flex min-h-screen flex-col items-center px-4 py-8">
    <div class="w-full max-w-md">
      ...
      <div v-if="!isPending && !user" class="py-20 text-center">
        ...
        <Button
          @click="
            authClient.signIn.social({ provider: 'google', callbackURL: '/challenges/create' })
          "
        >
