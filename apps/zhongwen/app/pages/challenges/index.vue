...
async function loadChallenges() {
  try {
    challenges.value = await $fetch("/api/challenges");
  } catch {
    /* ignore */
  } finally {
    loading.value = false;
...
/**
 * Join a challenge by invite code and navigate to it.
 */
async function joinByCode() {
  ...
  joinError.value = "";
  try {
    const result = await $fetch("/api/challenges/join", {
      method: "POST",
...
/**
 * Format remaining time or date range for display.
 */
  ...
  <div class="flex min-h-screen flex-col items-center px-4 py-8">
    <div class="w-full max-w-lg">
      ...
      <div v-if="!isPending && !user" class="py-20 text-center">
        ...
        <p class="mb-6 text-sm text-muted-foreground">Sign in with Google to create or join challenges.</p>
        <Button @click="authClient.signIn.social({ provider: 'google', callbackURL: '/challenges' })">
      ...
      <div class="mb-6 flex gap-2">
        ...
        />
        <Button :disabled="joining || !inviteCode.trim()" @click="joinByCode">
          Join
        </Button>
      </div>
      <p v-if="joinError" class="mb-4 text-sm text-destructive">{{ joinError }}</p>
        ...
        <section v-if="active.length" class="mb-6">
          ...
          <div class="flex flex-col gap-2">
            <Link
              ...
              :hover="true"
            >
              <div class="rounded-2xl border border-border bg-card p-4 transition-transform hover:-translate-y-0.5">
                <div class="flex items-start justify-between gap-2">
                  <div class="min-w-0 flex-1">
                    <p class="truncate text-sm font-semibold text-foreground">{{ c.title }}</p>
                    <p class="mt-0.5 text-xs text-muted-foreground">
                      HSK {{ c.hskLevel }} · {{ c.questionCount }} questions · {{ c.participantCount }} players
                    </p>
                  </div>
                  <span class="shrink-0 rounded-full bg-emerald-500/15 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">
                    {{ formatTimeInfo(c) }}
                  </span>
                </div>
                <div v-if="c.bestScore !== null" class="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
                  <span>Score: {{ c.bestScore }}/{{ c.questionCount }}</span>
                  <span v-if="c.rank !== null" class="rounded-full bg-primary/10 px-2 py-0.5 font-medium text-primary">
            ...
            >
              <div class="rounded-2xl border border-border bg-card p-4 transition-transform hover:-translate-y-0.5">
                <div class="flex items-start justify-between gap-2">
                  ...
                  <span class="shrink-0 rounded-full bg-amber-500/15 px-2 py-0.5 text-xs font-medium text-amber-600 dark:text-amber-400">
                    {{ formatTimeInfo(c) }}
                  </span>
                </div>
              </div>
        ...
        <section v-if="past.length">
          ...
          <div class="flex flex-col gap-2">
            <Link
              ...
              :hover="true"
            >
              <div class="rounded-2xl border border-border bg-card p-4 opacity-70 transition-transform hover:-translate-y-0.5">
                <div class="flex items-start justify-between gap-2">
                  <div class="min-w-0 flex-1">
                    <p class="truncate text-sm font-semibold text-foreground">{{ c.title }}</p>
                    <p class="mt-0.5 text-xs text-muted-foreground">
